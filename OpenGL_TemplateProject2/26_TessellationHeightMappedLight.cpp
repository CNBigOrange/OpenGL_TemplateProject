...
using namespace std;
namespace TessellationHeightMappedLight {
...
using namespace TessellationHeightMappedLight;

void installLightsHeightMapped(glm::mat4 vMatrix) {
...
void init_TessellationHeightMappedLight(GLFWwindow* window) {
	renderingProgram2 = createShaderProgram(TESSELLATION_HEIGHT_MAPPED_LIGHT, ENABLE_TESSELLATION);
...
void display_TessellationHeightMappedLight(GLFWwindow* window, double currentTime) {
	...
	installLightsHeightMapped(vMat);
...
int main26(void) {
	...
	init_TessellationHeightMappedLight(window);

	while (!glfwWindowShouldClose(window)) {
		display_TessellationHeightMappedLight(window, glfwGetTime());
