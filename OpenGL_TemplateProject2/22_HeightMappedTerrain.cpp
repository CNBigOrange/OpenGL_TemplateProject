...
#define numVBOs 5
	...
	float lightPos[3];
	GLuint roofTexture, moonTexture;

	// white light
	float globalAmbient[4] = { 1.3f, 1.3f, 1.3f, 1.0f };
...
using namespace HeightMapping;
void installLights_HeightMapping(glm::mat4 vMatrix) {
...
void setupVertices_light_HeightMapping(void) {
...
void init_light_HeightMapping(GLFWwindow* window) {
	renderingProgram2 = createShaderProgram(HEIGHT_MAPPING);
	...
	setupVertices_light_HeightMapping();
	roofTexture = loadTexture("texture/heightTexture.jpg");
	moonTexture = loadTexture("texture/height.jpg");
}

void display_light_HeightMapping(GLFWwindow* window, double currentTime) {
	...
	mMat = glm::scale(mMat, glm::vec3(3.0, 3.0, 3.0));
	...
	installLights_HeightMapping(vMat);
...
int main22(void) {
	...
	init_light_HeightMapping(window);

	while (!glfwWindowShouldClose(window)) {
		display_light_HeightMapping(window, glfwGetTime());
