...
using namespace geomModChange;
void installLightsGeomModChange(glm::mat4 vMatrix) {
...
void setupVertices_GeomModChange(void) {
...
void init_lightGeomModChange(GLFWwindow* window) {
	//renderingProgram2 = Utils::createShaderProgram("vertShaderGeomModInflate.glsl", "fragShaderGeomModInflate.glsl");
	renderingProgram2 = createShaderProgram(GEOM_MOD_INFLATE_CHANGE,ENABLE_GEOMETRY_ONLY);
	...
	setupVertices_GeomModChange();
}

void display_GeomModChange(GLFWwindow* window, double currentTime) {
	...
	installLightsGeomModChange(vMat);
...
int main30(void) {
	...
	init_lightGeomModChange(window);

	while (!glfwWindowShouldClose(window)) {
		display_GeomModChange(window, glfwGetTime());
