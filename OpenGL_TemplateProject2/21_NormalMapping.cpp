...
using namespace normalMappingMoon;
void installLights_normal_Moon(glm::mat4 vMatrix) {
...
void setupVertices_light_normal_Moon(void) {
...
void init_light_normal_Moon(GLFWwindow* window) {
	renderingProgram2 = createShaderProgram(NORMAL_MAPPING_MOON);
	...
	setupVertices_light_normal_Moon();
	roofTexture = loadTexture("texture/moonNORMAL.jpg");
	moonTexture = loadTexture("texture/moon.jpg");
}

void display_light_normal_Moon(GLFWwindow* window, double currentTime) {
	...
	installLights_normal_Moon(vMat);
...
int main21(void) {
	...
	init_light_normal_Moon(window);

	while (!glfwWindowShouldClose(window)) {
		display_light_normal_Moon(window, glfwGetTime());
