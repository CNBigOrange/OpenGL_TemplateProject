...
#define numVBOs 5
	...
	//Torus myTorus(1.f, 0.5f, 48);
	Sphere myTorus(48);
	ImportedModel pyramid("models/pyr.obj");
...
void installLightsBlending(glm::mat4 vMatrix) {
...
void setupVertices_Blending(void) {
...
void init_lightBlending(GLFWwindow* window) {
	//renderingProgram2 = Utils::createShaderProgram("vertShaderGeomModInflate.glsl", "fragShaderGeomModInflate.glsl");
	renderingProgram2 = createShaderProgram(BLENDING);
	//renderingProgram2 = Utils::createShaderProgram("BlinnPhongShaders/vertShader.glsl", "BlinnPhongShaders/fragShader.glsl");
	cameraX = 0.0f; cameraY = 0.032f; cameraZ = 200.0f;
	torLocX = 1.0f; torLocY = 0.0f; torLocZ = -100.0f;

	glfwGetFramebufferSize(window, &width, &height);
	aspect = (float)width / (float)height;
	pMat = glm::perspective(1.0472f, aspect, 0.1f, 1000.0f);

	setupVertices_Blending();

}

void display_Blending(GLFWwindow* window, double currentTime) {
	...
	aLoc = glGetUniformLocation(renderingProgram2, "alpha");
	...
	mMat = glm::scale(mMat, glm::vec3(0.7 , 0.8, 0.6));
	...
	glProgramUniform1f(renderingProgram2, aLoc, 1.0f);
	...
	mMat = glm::rotate(mMat, toRadians(60 * currentTime), glm::vec3(1.0f, 1.0f, 0.0f));
	...
	glCullFace(GL_FRONT);
	glProgramUniform1f(renderingProgram2, aLoc, 0.3f);
	glProgramUniform1f(renderingProgram2, fLoc, -1.0f);
	glDrawArrays(GL_TRIANGLES, 0, numPyramidVertices);

	glCullFace(GL_BACK);
	glProgramUniform1f(renderingProgram2, aLoc, 0.7f);
	glProgramUniform1f(renderingProgram2, fLoc, 1.0f);
	glDrawArrays(GL_TRIANGLES, 0, numPyramidVertices);
...
int main(void) {
	...
	init_lightBlending(window);

	while (!glfwWindowShouldClose(window)) {
		display_Blending(window, glfwGetTime());
