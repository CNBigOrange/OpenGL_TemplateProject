...
using namespace MoreCube;

void setupVertices2(void) {//36个顶点，12个三角形，组成了放置在原点处的2x2x2立方体
...
void init3D_Cube2(GLFWwindow* window) {
...
void display3D_Cube2(GLFWwindow* window, double currentTime) {
	...
	pMat = glm::perspective(1.0472f, aspect, 0.1f, 3000.0f);//1.0472 radians = 60 degress
	...
	for (int i = 0; i < 24; i++) {
		...
		rMat = glm::rotate(glm::mat4(1.0f), 15.75f * (float)timeFactor, glm::vec3(1.0f, 1.0f, 1.0f));
		mMat = glm::translate(glm::mat4(1.0f), glm::vec3(sin(.35f* timeFactor)*8.0f, cos(.52f * timeFactor) * 8.0f, cubeLocZ+sin(.70f * timeFactor) * 8.0f));
		mMat = mMat * rMat;
		mvMat = vMat * mMat;
...
int main7() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		display3D_Cube2(window, glfwGetTime());
