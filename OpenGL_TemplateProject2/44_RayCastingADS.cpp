...
using namespace RayCastingADS;

void init_RayCastingADS(GLFWwindow* window) {
	...
	raytraceComputeShader = Utils::createShaderProgram("RayCastingADSComputeShader.glsl");
	screenQuadShader = Utils::createShaderProgram("RayCastingVertShader.glsl", "RayCastingFragShader.glsl");
}

void displayRayCastingADS(GLFWwindow* window, double currentTime) {
	//=======================================================
...
int main44(void) {
	...
	init_RayCastingADS(window);

	while (!glfwWindowShouldClose(window)) {
		displayRayCastingADS(window, glfwGetTime());
