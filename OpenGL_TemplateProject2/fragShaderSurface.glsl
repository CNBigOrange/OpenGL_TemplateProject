...
vec3 estimateWaveNormal(float offset, float mapScale, float hScale)//传入参数是临近程度的缩放因子、相对于场景的噪声图尺寸和高度
{	// estimate the normal using the noise texture判断波浪法线
...
{	vec4 fogColor = vec4(0.0, 0.0, 0.2, 1.0);
	...
	vec3 N = estimateWaveNormal(.001, 10.0, 16.0);
	...
	{	float fresnel = acos(cosFres);
		fresnel = pow(clamp(fresnel-0.61, 0.0, 1.0), 3.0);//为特定场景需要微调菲涅尔的减值
